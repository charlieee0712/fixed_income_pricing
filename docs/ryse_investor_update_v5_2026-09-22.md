# Ryse — investor progress update, version 5

**Source for `docs/Ryse Presentation v5.pptx`.** Edit this, then run
`python scripts/make_investor_deck.py --input docs/ryse_investor_update_v5_2026-09-22.md
--output "docs/Ryse Presentation v5.pptx"`.

**Presenting tomorrow:** Liping takes slides **2–5**, Lichen takes **6–9**.

Version 4 is Liping's plain-language pass over version 3 and most of it is kept verbatim.
This version changes four things and leaves the rest of her wording alone — the full list
is at the end.
...
exactly, at 2,366. If anyone asks about the last three rows: futures and options are not bonds,
they are priced by different machinery, and they are 17 securities out of 2,260.

[⚠️ VERSION 4 MOVED FUTURES AND OPTIONS FROM "OUT OF SCOPE" TO "SCOPED", AND THAT BROKE THE
DECK. The chart on slide 6 still counted them as out of scope, so the two adjacent slides
disagreed: 412 scoped against 428, and 17 out of scope against 1. Lichen presents both. Put
back to out of scope, which is what the project record says and what the chart counts.
**Ask Liping whether she had a reason** — if Mario has said derivatives are coming, we flip
the chart AND the table together, not one of them.]
...
slides 1, 2 and 5 are hers, and the improvements on 3 and 8 are hers in substance. Four changes:

| # | slide | change | why |
|---:|---|---|---|
| 1 | **7** | Futures and Options put back to **Out of scope** | ⭐ v4 marked them Scoped while the chart on slide 6 still counted them out of scope. The two adjacent slides disagreed — 412 against 428 scoped, 17 against 1 out of scope — and Lichen presents both |
...
- **Slide 7's last three rows.** If she marked futures and options "Scoped" for a reason we do
  not know about, say so and we change the chart to match. If it was a judgement call, out of
  scope is what the record says and what the chart counts.
