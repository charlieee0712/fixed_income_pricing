...
## Update 2026-08-31 — packaging is a FOLDER; there are no zips

**Every packaging zip is retired.** Google Drive uploads a folder directly, so zipping was pure
overhead. The deliverable is the git-ignored staging **folder** `code_structure_sample/`, which
the user drags to Drive. Both handoff bundles are folders too, for the same reason; their zips
are gone and are worth rebuilding only if claude.ai ever needs an archive rather than files.

Separately, the four tracked `fixed_income_code_v*.zip` whole-tree snapshots from June and July
were **removed from the branch** on 2026-08-31. History is untouched — `git show
<old-commit>:fixed_income_code_v3.zip` still recovers any of them — and `.gitignore` now carries
`/fixed_income_code_v*.zip`, anchored at the root so the tracked `data/*.zip` source archives
cannot match. They were removed because at the repo root they read as the *current* delivery
package, which they are not.

The package now carries **eight** numbered documents: `00_README` · `01_weekly_report` (md+pdf)
· `02_code_walkthrough` (md+pdf) · `03_json_excel_interface` · `04_previous_report` ·
`05_round_detail` · `06_delivery_evidence` · **`07_release_facts`** — the last being the
sha256 and row count of every output file quoted anywhere in the package, generated by
`scripts/release_facts.py` from the files themselves.

⚠️ **The package deliberately carries the code and not the portfolio.** Running `pytest` inside
it gives `73 failed, 277 passed, 35 skipped, 5 errors` — every red result a `FileNotFoundError`
on a data file it does not duplicate. The README says so, with that exact line, so that what a
reader runs matches what they read. The 277 that do run are the model invariants, the
cross-engine arithmetic identities, the request/response contract and the structural locks —
none of which need a bond.
