...
Seven decisions locked since the last refresh, and **one previously-recorded claim
retracted**.

| # | decision | date | why, and what would reopen it |
|---|---|---|---|
| L37 | **Discount on the OWN-CURRENCY curve, never per-country** | 09-03 | Matches legacy `zeroyield4(ccy)` and the corporate book. ⭐ The decider was **coverage, not taste**: Ireland has no curve file, so a per-country rule splits 2 Irish holdings from 28 euro peers. Per-country stays the cross-check. Fable consulted before any code |
| L38 | **`TITLE_FACE` is an explicit per-currency registry, never a price sniffer** | 09-03 | The quotation identity holds for MXN 100 and BRL 1000 alike, so it **cannot** name the denomination. The `PAR_YIELD_UNITS` lesson applied in advance rather than after a two-month misdiagnosis. Reopens when a new non-100 currency arrives |
| L39 | **Guaranteed Fixed Income migrates WITH agencies and linkers** | 09-10 | They share one loader, one driver and one 63-row hashed CSV. Migrating two of three leaves an un-migrated island inside migrated code |
| L40 | **No ILB endpoint type** | 09-10 | The worksheet has no cells for a real coupon, an index ratio or an inflation assumption, and the 7/5/5 rule keeps a type off a sheet whose layout Mario has not chosen. `contracts.py`, `schema_version` (1.1) and the `.bas` untouched. ⭐ **Reopens the day Mario names the cells** |
| L41 | **`embedded_option.py` stays in `assets/corporate/`; government gets an import path** | 09-10 | It is **mis-located, not mis-written** — one lattice prices every optioned bond in the book, including a US Treasury. Moving it would touch the corporate hashed CSVs. Reopens on a third non-corporate caller, or the securitized layer |
| L42 | **Cross-platform tolerance is PER-QUANTITY** — convexity 1e-6, everything else 1e-10 | 09-13 | ⭐ One bound sized for the noisiest quantity let a deliberate 1e-4 price perturbation through. The amplification belongs to the second-difference formula, not to the endpoint. Mutation-verified |
| L43 | **The text fingerprint is standard-library only** | 09-15 | A check meant to be invariant across platforms cannot rest on a library whose behaviour varies across versions. Its pandas version broke on the first machine it met. A test forbids importing pandas or numpy in that module |

### ⚠️ Retracted: "the endpoint's JSON output is byte-for-byte the same as 47's"

Recorded in CLAUDE.md on 2026-08-25 and **true when written** — the endpoint priced
vanilla bonds only. The lattice products arrived on 08-31 and nobody re-checked. It was
false from that day until 2026-09-13.

Strings, codes, field names and structure ARE identical everywhere. **`convexity` differs
by up to 7.5e-08.** Full measurements in `24`.

### Still in force and worth restating, because plans keep drifting from them

* **Excel scope is 8 / 7 / 5 / 5** since 09-10 — products priced / contract types /
  constructible by the VBA builder / verified by a real-Excel round trip. Never "the
  spreadsheet can ask for any of seven".
* **The delivery package ships `data/`** (user, 09-05). It is Mario's own material coming
  back to him, so `pytest -q` gives the full count there. ⚠️ The old README line "this
  folder carries the code, not the portfolio" is **retired — do not restore it**.
* **Mario's judgement, relayed 09-15: the client data is not sensitive.** That settles the
  sensitivity question for Azure. It does not settle *which tenant*.
