...
## 2. The suite, 223 checks *(as of 2026-08-25 — now **390**; this section is kept for the
per-area reasoning, and the current inventory is the 08-31 update at the end of this file)*
...
## Update 2026-08-31 — 390 tests, and two checks that look unusual on purpose

**287 → 390.** The suite is the gate; the five production CSVs are the freeze
(`docs/release_facts_<date>.md` carries rows + sha256 for all five, plus the four disposition
sidecars and the real pytest line, generated by `scripts/release_facts.py` from the files).

| file | n | what it guards |
|---|---:|---|
| `test_exception_wiring.py` | **73** | see below — a STATIC check, not a runtime one |
| `test_json_endpoint_dispatch.py` | 38 | seven-type dispatch, provenance warnings, the Excel-scope pin |
| `test_pricer_floating_structure.py` | 32 | the floating/hybrid structure, and the running-coupon freeze |
| `test_pricer_tree_structure.py` | 30 | the tree wrappers, exercise refusals, representability |
| `test_lattice.py` | 29 | the BDT invariants |
| `test_vanilla_json_endpoint.py` | 28 | the v1.0 contract, unchanged |
| `test_callable_disposition.py` | 18 | set-level disposition, terms provenance, the defaulted rule |
| `test_price_convention.py` | 16 | the clean/dirty law across every engine |
| `test_bootstrap.py` · `test_mbs.py` · `test_monthly_curves.py` | 13 each | |
| `test_hybrid.py` · `test_term_overrides.py` · `test_universe.py` | 11 each | |
| `test_coupon_schedule.py` | 10 | |
| `test_frn.py` · `test_ilb.py` · `test_ratings.py` | 7 each | |
| `test_phase2_universe.py` | 6 | |
| `test_call_schedules.py` | 5 | now pins ACT/364 and that `days_per_year` is a `TypeError` |
| `test_oas.py` | 4 | |
| `test_pricer_structure.py` | 8 | |

### Two checks worth understanding before you plan around them

**`test_exception_wiring.py` is static, and that is the point.** It parses every file in
`src/` and `scripts/` with `ast` and requires each name in an `except` clause to be bound
somewhere in that module. A runtime check would be worthless here: an
`except (CalibrationError, ValueError)` whose `CalibrationError` was never imported is
perfectly valid Python until an exception passes through it. `scripts/phase2_risk.py` shipped
in exactly that state, and all five drivers ran green because no bond failed calibration that
day. It is parametrised per file, so its count moves whenever a file is added — 73 today.
It carries its own non-vacuity lock: a test proving the checker detects that exact shape.

**The Excel-scope pin greps a `.bas` file.** `test_the_excel_bridge_emits_only_the_documented_fields`
asserts which `bond.*` fields `RysePricingBridge.bas` emits, so the documented 7 / 5 / 5 cannot
go stale silently. It is *designed* to fail when the bridge grows a field, and its docstring
names the four documents to update. It did exactly that on 08-31 when the floating cells landed.

### The parity protocol, restated
