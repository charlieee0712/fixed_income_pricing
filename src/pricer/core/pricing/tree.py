"""Black-Derman-Toy-style short-rate lattice for bonds with embedded options
(template: ``core/pricing/tree.py``) — callable / putable OAS + risk (v2).

MIGRATED VERBATIM from ``pricing/lattice.py`` (2026-08-25, Round 2 Gate 1): the numerical
code below is byte-for-byte the validated implementation, so float-operation order — and
therefore every production number — is unchanged. ``pricing.lattice`` is now a shim that
re-exports these objects. ONE tree serves callable, puttable and (Gate 3) sinking-fund
bonds; the asset wrappers in ``assets/corporate/`` add legacy units and schedules only.
...
def bond_tree(valuation_date, maturity, coupon_rate, curve, freq: int = 2,
              sigma: float = 0.15):
    """Build the calibrated lattice for one real bond, on its true coupon dates.
    ...
    4. curve          : ZeroCurve — the bond's own-currency discount curve.
    5. freq           : int — coupon payments per year.
    6. sigma          : float — short-rate volatility, DECIMAL (0.15 = 15%).

    Returns: ``(lattice, accrued)`` — a :class:`ShortRateLattice` on the REAL remaining
    coupon dates (so its root value is the true DIRTY price) and the shared ACT/364
    accrued to subtract for a clean price. This is the production grid convention fixed in
    the 2026-08-04 review; do not price a dated bond on the regular synthetic grid.
    """
    coupon_times, accrued = lattice_inputs(valuation_date, maturity, coupon_rate, freq=freq)
    return ShortRateLattice(curve, freq=freq, sigma=sigma, coupon_times=coupon_times), accrued
