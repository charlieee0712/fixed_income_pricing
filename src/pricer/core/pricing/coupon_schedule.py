"""Coupon time-tables for stepped / step-up / date-segmented-fixed bonds
(template: ``core/pricing/coupon_schedule.py``).

MOVED VERBATIM from ``pricing/coupon_schedule.py`` (Round 2b, 2026-08-30) — the
parsing and lookup logic is byte-for-byte the code that produced every published
stepped/step-up number; ``pricing.coupon_schedule`` is now a shim over this module.
The move also removes a layering violation: ``core/pricing/cashflows.py`` used to
import ``coupon_at`` from the legacy ``pricing`` package, i.e. core reached upward
into a not-yet-migrated layer. It now imports a sibling.

Which bonds this serves (Mario's "Pivot of Corp Bonds" sheet, column F):
  row 13  "7.00% for t<01-Mar-2006 7.50% for t>=01-Mar-2006"  (2 rows) — segmented fixed
  row 20  "Step-up schedule"                                  (1 row)  — step-up
Both route to the ordinary discounting engine once it is handed a coupon
time-table instead of a single rate; they are NOT a separate pricing model.
