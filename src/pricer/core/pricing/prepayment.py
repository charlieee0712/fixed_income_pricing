"""Constant-prepayment pool mathematics — CPR/SMM amortisation and pool pricing.

Moved here from ``pricing/mbs.py`` (2026-09-25) under the code-structure directive; it was the
last engine still outside the template. **The body below the docstring is the file that was
written 2026-07-22, spliced unchanged** — sha256 ``45a0ef7d5d98d893``, asserted by
``tests/test_pricer_securitized_structure.py``. ``pricing/mbs.py`` is now a shim over it.

Named for the mathematics rather than the product, like its siblings (analytical / tree /
floating / hybrid / inflation): what lives here is prepayment-adjusted amortisation, which is
what CMOs, ABS and CMBS will reuse. The product wrappers live in ``assets/securitized/``.
...
Static CPR means the cash flows do NOT respond to rates, so the risk metrics are
spread-durations of fixed flows; the CPR(rate) response (negative convexity) is the v2
prepayment model.

⭐ ONE PRICE CANNOT IDENTIFY CPR AND OAS TOGETHER, and which one you solve for is now a
settled decision rather than an open option. The delivered Bloomberg pull (checked 2026-09-25,
``scripts/mbs_data_check.py``) is as-of 2026, so its trailing CPRs describe the wrong decade
and cannot be an input for a 2009 valuation. The adopted route is the other branch: fix the
spread near zero for government-guaranteed paper and let the custodian price imply the CPR via
:func:`implied_cpr_pool`. :func:`implied_spread_pool` remains for the day a 2009-dated CPR
arrives — then the two become a cross-check on each other.
