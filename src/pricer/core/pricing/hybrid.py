"""Fixed-then-float hybrid pricing (template: ``core/pricing/hybrid.py``) — the two
validated engines glued at the switch date.

MOVED VERBATIM from ``pricing/hybrid.py`` (Round 2b, 2026-08-30): only the two import
lines changed, from the legacy paths to their pricer-native equivalents. Both are exact
aliases of the same objects (``pricing.bond_price.price_bond`` IS
``core.pricing.analytical.price_fixed_rate_bond``), so the arithmetic is unchanged;
``pricing.hybrid`` is now a shim over this module.

Which bonds this serves (Mario's "Pivot of Corp Bonds" sheet, column F):
  row 12  "Fixed -> Floating"          (5 tab rows)
and, though Mario did not mark them, the same engine serves rows 6-11
("Fixed -> Reset ...", 6 tab rows) — a reset hybrid is the same instrument shape.
