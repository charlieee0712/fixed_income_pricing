"""Inflation-linked government bond (ILB / TIPS) pricing + calibration (template:
``core/pricing/inflation.py``).

MOVED VERBATIM from ``pricing/ilb.py`` (2026-09-10) — byte-for-byte the code that produced every
published inflation-linked number; ``pricing.ilb`` is now a shim over this module. Originally
built for phase 2 (inventory 2026-07-20, engine 2026-07-22).

Which securities this serves — the master's ``Index Linked Government Bonds`` sub-category, one
of the two classes in Mario's directive of 2026-09-10:
  16 master rows -> 15 securities -> 14 priced here; 1 (the Korean KTBi) is carried at the
  custodian mark because its index ratio is not derivable — see ``dataio.phase2``.
The per-metric wrappers with legacy naming live in ``pricer.assets.government.linker``.

Named ``inflation`` rather than ``ilb`` because ``core/`` is named for the mathematics
(``analytical``, ``tree``, ``floating``, ``hybrid``, ``discounting``) while ``assets/`` is named
for the product; "ILB" is a product name and belongs on the wrapper.
